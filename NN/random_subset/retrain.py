...
def get_dataloader(args, ):
    ...
    retainDS_inTrainSet = dataSet(mode="train", transform=transform_train, forget_idx=random_subset_idx,
                                  isForgetSet=False, )
    ...
    retainDL_inTrainSet = DataLoader(retainDS_inTrainSet, batch_size=args.batchsize, shuffle=True, num_workers=8,
                                     pin_memory=True)
...
def test(epoch, net, testloader, device, criterion, best_acc, log: LogProcessBar, best_model, last_model, args,
         msg="Test", save=True, forward=None):
...
if __name__ == '__main__':
    ...
    retainDL_inTrainSet, retainDL_inTrainSet_woDataAugment, forgetDL_inTrainSet_woDataAugment, testDL = get_dataloader(
        args)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        test_acc, best_acc, best_model, last_model = test(
            epoch, net, testDL, device, criterion, best_acc, log, best_model, last_model, msg="Test", save=True,
            args=args)
        _, forget_acc = valid_test(net, forgetDL_inTrainSet_woDataAugment, "Forget Acc", device, criterion, log,
                                   args=args)
    ...
    mia_normal = get_membership_attack_prob(retainDL_inTrainSet_woDataAugment, forgetDL_inTrainSet_woDataAugment,
                                            testDL, net)
    mia_m = get_membership_attack_prob_m(retainDL_inTrainSet_woDataAugment, forgetDL_inTrainSet_woDataAugment, testDL,
                                         net)
    log.log_print(
        "Test acc:{:.3f}\tForget acc:{:.3f}\tMIA:{:.3f}\tM_MIA:{:.3f}".format(test_acc, forget_acc, mia_normal, mia_m))
