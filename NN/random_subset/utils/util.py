...
class DefaultModel(nn.Module):
    ...
    def takefeature(self, x):
        if self.data_normalize is not None:
            x = self.data_normalize(x)
        return self.net.takelastfeauture(x)

    def get_features(self, x):
        if self.data_normalize is not None:
            x = self.data_normalize(x)
        return self.net.get_features(x)
...
def update_ckpt(args):
    basic_dir = "tmp/{}/{}/{}".format(args.model, args.ckpt.replace(".pth", ""),
                                      datetime.datetime.now().strftime('%y_%m_%d_%H_%M_%S.%f')[:-4])
    if not args.debug:
        config = vars(args)

        for key in ["dataset", "opt", ]:
            if key in config:
                basic_dir += "_{}".format(config[key])

        for key in ["lr", "epoch", "batchsize", "wd", "threshold", "std", "delta", "forget_per"]:
            if key in config:
