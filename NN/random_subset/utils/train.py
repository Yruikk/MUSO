...
def train_rwp_salun(net, trainloader, scheduler, optimizer: optim.Optimizer, criterion, log: LogProcessBar, device,
                    args, std=0.01, mask=None):
    net.train()
    ...
    for batch_idx, batch_data in enumerate(trainloader):
        ...
        if args.debug or (batch_idx == len(trainloader) - 1): log.refresh(batch_idx, len(trainloader), msg)
    return 100. * correct_sample.sum / total_sample.sum
...
def valid_test(net, dataloader, mode: str, device, criterion, log: LogProcessBar, args, forward=None, ):
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        if args.debug or (batch_idx == len(dataloader) - 1): log.refresh(batch_idx, len(dataloader), msg)
    return train_loss.avg, 100. * correct_sample.sum / total_sample.sum


@torch.no_grad()
def valid_test_for_yu(net, right_side, dataloader, mode: str, device, criterion, log: LogProcessBar, args, forward=None):
    if forward is None:
        forward = net.forward
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        ...
        Z_u = net.takefeature(inputs)  # N x 512
        all_ones = torch.ones(Z_u.shape[0], 1).to(device)
        Z_u_wbias = torch.cat([Z_u, all_ones], dim=1)  # N x (512+1)
        y_u = Z_u_wbias @ right_side  # N x 10
        y_u_probs = torch.softmax(y_u, dim=1)

        # pred_y = net(inputs)
        # y_u_probs = torch.softmax(pred_y, dim=1)

        outputs = y_u_probs.float()

        # loss = criterion(outputs, targets)

        total_y = F.one_hot(targets, 10).float()
