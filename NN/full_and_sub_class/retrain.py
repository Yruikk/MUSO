...
if __name__ == '__main__':
    ...
    device = auto_select_device(memory_max=8000, memory_bias=200, strategy='greedy')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        _, forget_acc_on_train = valid_test(net, forgetDL_inTrainSet_woDataAugment, "Forget Acc on Train", device,
                                            criterion, log, args=args)
    ...
    mia = get_membership_attack_prob(retainDL_inTrainSet_woDataAugment, forgetDL_inTrainSet_woDataAugment, testDL, net)
    log.log_print("Retain Acc:{:.3f}\tForget Acc on Train:{:.3f}\tForget Acc on Test:{:.3f}\tMIA:{:.3f}".format(
        retain_acc, forget_acc_on_train, forget_acc_on_test, mia))
    last_model = {
        'net': deepcopy(net.state_dict()),
        'epoch': epoch,
    }
