...
def get_gpu_memory_map():
    ...
    # gpu_memory = np.array([int(x) if x.isdigit() else float('inf') for x in result.strip().split('\n')])
    result = result.strip().split('\n')
    Fan = Fan.strip().split('\n')
    gpu_memory = np.array([int(result[i]) if Fan[i].isdigit() else float('inf') for i in range(len(result))])
    return gpu_memory
